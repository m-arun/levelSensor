...
uint8_t flag = 0, uartMsgLen = 0;
uint16_t vdd = 0, D1 =0, D2 =0;
...
float water_level1=0, temperature1 = 0;
float TF = 0, R=0, voltage = 0, a = 0, b = 0;
...
static void PrepareTxFrame(uint8_t port) {
	...
	case 3:
		...
		water_level1 = (D1*80.0)/1000.0;
		voltage = (D2*3.3)/16384.0;
		R = (voltage*10000.0)/(3.3 - voltage);
		a = log10(R/10000.0)/3950.0;
		TF = a + (1/298.15);
		temperature1 = (1/TF) - 273.15;

        WaterSensor ws= {timestamp, true, water_level1, true, temperature1, true, vdd};
        pb_ostream_t stream = pb_ostream_from_buffer(buffer, sizeof(buffer));
        pb_encode(&stream, WaterSensor_fields, &ws);
...
int main(void) {
	...
	while (1) {
		if ( flag == 1){
			flag = 0;
			UartGetBuffer(&Uart1, rxBuffer, 100, &uartMsgLen);

			token = strtok(rxBuffer, ","); // Gives Type
			d1 = strtok(NULL, ","); // Gives ID
			d1 = strtok(NULL, ","); // Gives D1
			d2 = strtok(NULL, ","); // Gives D2
